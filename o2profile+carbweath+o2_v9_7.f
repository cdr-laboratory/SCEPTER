      ...
      double precision :: po2i = 0.21d0 ! atm **default
C       double precision :: po2i = 0.6d-1 ! atm
      double precision :: pco2i = 10.0d0**(-2.5d0) ! atm **default 
...
C       double precision :: redsldi = 0.56d0 ! wt%  **default 
C       double precision :: redsldi = 1.12d0 ! wt%  x2
      double precision :: redsldi = 2.8d0 ! wt%   x5
C       double precision :: redsldi = 2.24d0 ! wt%  x4
C       double precision :: redsldi = 3.36d0 ! wt%  x6
      
      double precision :: silwti = 30d0 ! wt%  **default
C       double precision :: silwti = 45d0 ! wt%  
...
C       double precision :: zsat = 5d0
      ...
      double precision :: dna  = 1.90d-2   ! m^2 yr^-1 ! at 15 C; Li and Gregory 
      
      double precision, parameter :: w = 5.0d-5 ! m yr^-1, uplift rate ** default 
...
C       double precision, parameter :: qin = 5d-3 ! m yr^-1, advection (m3 water / m2 profile / yr)
      ...
      double precision :: qin = 1d-1 ! m yr^-1, advection (m3 water / m2 profile / yr)  ** default
C       double precision :: qin = 2d-1 ! m yr^-1 
      double precision v(nz), q
      
C       double precision :: hr = 1d5 ! m^2 m^-3, reciprocal of hydraulic radius  ** default 
C       double precision :: hr = 1d4 ! m^2 m^-3, reciprocal of hydraulic radius
      double precision :: hri = 1d5
      
      double precision :: hr(nz)
      ...
      double precision hco3(nz), ca(nz), co2(nz), co3(nz), dic(nz)
      double precision porox(nz), dporodta(nz),dporodtg(nz)
      
      double precision :: caeq = 1d-3  ! mol/L equilibrium Ca conc. 
      double precision :: delca = 0.5d0  ! m reaction front width  
      double precision :: zca = 50d0   ! m depth of reaction front for calcite          
      ...
      double precision rxnx(nz),rxn(nz),drxnx_dnax(nz),drxnx_dprox(nz)
      
      double precision kho, ucv
      double precision kco2,k1, keqsil, kw, k2, kcceq
...
!       double precision time2(nt2)
     ...
     $  , fepy2flx(2), fetflx(2), fediflx(2)
      ...
      double precision fpro(nz),dfpro(nz), fa(nz),fb(nz),fc(nz),fd(nz)
      double precision error_pro, tol_pro, dprox(nz)
...
C       rectime=rectime*3d0 
      ...
      if (tc/=15d0) then 
         qin = qin*exp(-30d0*(1.0d0/(273.0d0+tc)
...
C       write(workdir,*) write(workdir,*) '/home/latruffe/PyWeath/'         
      write(runname,*) 'Fe2+SO4+carb+ph_wet_iter'
     $   //'---q'//trim(adjustl(chrq(3)))//'_zsat'
...
#endif
#ifdef carbweath
      write(runname,*) 'carb+ph_wet_iter'
     $   //'---q'//trim(adjustl(chrq(3)))//'_zsat'
     $   //trim(adjustl(chrz(3)))//trim(adjustl(base))
C      $   //'_co21e-1-o26e-2'
#endif
...
C       print *,pro(1);stop
      ...
      ksil = 1.93d-5*1d4  ! mol/m2/yr  ! from Li et al., 2014
      ksil = 1d-8
      keqsil = kcceq
...
C       print *,keqsil;stop
      ...
      msili = silwti*1d-2/100.089d0*2.7d0*(1.0d0-poroi)*1d6
      ! 100.089 g mol^-1 and 36.934 cm3 mol^-1 (Robie et al., 1978) for calcite and 2.7 g cm^-3 for solid density
...
!  ############## pH interation #######################
     ...
     $  -1.0d0/(273.0d0+15.0d0))/rg)
      ...
      ksil = 1.31d-9*1d4  ! mol/m2/yr  ! from Li et al., 2014
      ksil = 1.93d-5*1d4  ! mol/m2/yr  ! from Li et al., 2014
      if (time < maxval(rectime)*0.001d0) ksil = 1d-6
      ksil = 1d-3
      ...
      pyrxn = 0d0
      do iz=1,nz
      pyrxn(iz)=koxs(iz)*poro(iz)*hr(iz)*23.94d0*1d-6*ms(iz)*
     $  po2(iz)**0.50d0
...
!       print *, iz,c2x(iz)
     ...
     $   *(c(iz)-c(iz-1))/(dz**2d0)
C      $   -(v(iz)-v(iz-1))*c2(iz)/dz
     $  +15d0*koxs2(iz)/sat(iz)*hr(iz)*23.94d0*1d-6*msx(iz)*
     $           c2x(iz)**0.93d0*c(iz)**(-0.40d0)
     $    *1d-3
     $  +koxs(iz)/sat(iz)*hr(iz)*23.94d0*1d-6*ms(iz)*po2(iz)**0.50d0
     $   *1d-3
     $             )
     $             )
      else 
...
C      $   -(v(iz)-v(iz-1))*c2(iz)/dz
     $  +2d0*koxs2(iz)/sat(iz)*hr(iz)*23.94d0*1d-6*msx(iz)*
     $           c2x(iz)**0.93d0*c(iz)**(-0.40d0)
     $    *1d-3
     $  +2d0*koxs(iz)/sat(iz)*hr(iz)*23.94d0*1d-6*ms(iz)*po2(iz)**0.50d0
     $   *1d-3
     $             )
     $             )
      else 
...
C      $   -(v(iz)-v(iz-1))*c2(iz)/dz
     $  +ksil(iz)/sat(iz)*hr(iz)*36.934d0*1d-6*msilx(iz)*1d-3
     $             )
     $             )
      else 
      nax(iz) = max(0.0d0,
...
C        pause
     ...
     $      + w/dz *(1.0d0-swex)   
     $     + (1.0d0-frex)*
     $  koxs(iz)*poro(iz)*hr(iz)*23.94d0*1d-6
...
C         if (iz /= 1) then
          ...
          amx(row,row + 2 ) = (
     $    + (1.0d0-frex)*merge(0.0d0,
     $   koxs(iz)*poro(iz)*hr(iz)*23.94d0*1d-6*msx(iz)
...
!         if (row == 188) write (*,*) row, it, iz, ymx(row)
     ...
     $   - (1.0d0-frex)*merge(0.0d0,
     $  koxs(iz)/sat(iz)*hr(iz)*23.94d0*1d-6*msx(iz)*0.50d0
     $           *(1d0-swoxall)
     $    *po2x(iz)**(-0.50d0),
     $   po2x(iz) <po2th.or.isnan(po2x(iz)**(-0.50d0)))*1d-3 
     $                    )
...
C         if (iz /= nz) then
          ...
          amx(row,row  - 1) = (     
     $   - (1.0d0-frex)*
     $   koxs(iz)/sat(iz)*hr(iz)*23.94d0*1d-6
     $           *(1d0-swoxall)
     $   *merge(0d0,po2x(iz)**(0.50d0),
     $    po2x(iz) <po2th.or. isnan(po2x(iz)**(0.50d0)))*1d-3 
     $   
...
!      $   po2x(iz) < po2th)
!      $  +merge(0.0d0,
!      $   stoxs*koxs(iz)*poro(iz)*hr(iz)*sat(iz)
!      $      *23.94d0*1d-6*msx(iz)*po2x(iz)**(0.50d0),
!      $   po2x(iz) < po2th)
!      $  +merge(0.0d0,
!      $   swbr*vmax*po2x(iz)/(po2x(iz)+mo2),
...
!      $     po2x(iz)< po2th)
     ...
     $   (1.0d0-frex)*koxs(iz)*poro(iz)*hr(iz)*stoxs
     $      *23.94d0*1d-6
     $   *merge(0d0,po2x(iz)**(0.50d0),
     $   po2x(iz) <po2th.or.isnan(po2x(iz)**(0.50d0)))
     $                            )
...
C      $  + swex*(v(iz)-v(iz-1))*c(iz)/dz
     ...
     $   
     $  -(1.0d0-frex)*merge(0.0d0,
     $   (2.0d0)*koxs2(iz)/sat(iz)*hr(iz)*23.94d0*1d-6*msx(iz)*
     $           c2x(iz)**0.93d0*cx(iz)**(-0.40d0),
     $    cx(iz)<cth.or.isnan(c2x(iz)**0.93d0*cx(iz)**(-0.40d0)))*1d-3
     $  -frex*merge(0.0d0,
     $   (2.0d0)*koxs2(iz)/sat(iz)*hr(iz)*23.94d0*1d-6*ms(iz)*
     $           c2(iz)**0.93d0*c(iz)**(-0.40d0),
...
C       prox = 1d-1
      ...
      if (it ==0) then 
      prox(:) = 0.5d0* (
     $   -1d0*(2d0*cx(:)+2d0*nax(:)+3d0*c2x(:)-2d0*so4x(:)) 
     $ + sqrt((2d0*cx(:)+2d0*nax(:)+3d0*c2x(:)-2d0*so4x(:))**2d0 
     $ + 4d0*kco2*k1*pco2i)
     $                          )
      else 
      
      prox(:) = 0.5d0* (
     $   -1d0*(2d0*nax(:)+2d0*cx(:)+3d0*c2x(:)-2d0*so4x(:)) 
     $ + sqrt((2d0*nax(:)+2d0*cx(:)+3d0*c2x(:)-2d0*so4x(:))**2d0 
     $ + 4d0*kco2*k1*pco2i)
     $                          )
C       print*,prox
      call calcpH(nz,nax,cx,c2x,so4x,k1,kco2,pco2i,k2,prox)
C       print*,prox
C       stop
      
      rxn(:) =
     $   (1d0-na(:)*k2*k1*kco2*pco2i/pro(:)/pro(:)/kcceq)
     $   *merge(0d0,1d0
     $  ,1d0-na(:)*k2*k1*kco2*pco2i/pro(:)/pro(:)/kcceq < 0d0)
...
C       if (iter/=0) then 
     ...
     $   -1d0 
     $ + 0.5d0/sqrt(
     $  (2d0*nax(:)+2d0*cx(:)+3d0*c2x(:)-2d0*so4x(:))**2d0 
     $ + 4d0*kco2*k1*pco2i)*2d0
     $  *(2d0*nax(:)+2d0*cx(:)+3d0*c2x(:)-2d0*so4x(:))*2d0
     $                          )
     
      call calcpH(nz,nax*(1d0+1d-6),cx
     $      ,c2x,so4x,k1,kco2,pco2i,k2,dprodna)
      
      dprodna(:) = (dprodna(:) - prox(:))/nax(:)/1d-6
      
      rxnx(:) = 
     $   (1d0-nax(:)*k2*k1*kco2*pco2i/prox(:)/prox(:)/kcceq)
     $   *merge(0d0,1d0
     $  ,1d0-nax(:)*k2*k1*kco2*pco2i/prox(:)/prox(:)/kcceq < 0d0)
      
      drxnx_dprox = 
     $   (2d0*nax(:)*k2*k1*kco2*pco2i/prox(:)/prox(:)/prox(:)/kcceq)
     $   *merge(0d0,1d0
     $  ,1d0-nax(:)*k2*k1*kco2*pco2i/prox(:)/prox(:)/kcceq < 0d0)
      
      drxnx_dnax = 
     $   (-1d0*k2*k1*kco2*pco2i/prox(:)/prox(:)/kcceq)
     $   *merge(0d0,1d0
     $  ,1d0-nax(:)*k2*k1*kco2*pco2i/prox(:)/prox(:)/kcceq < 0d0)
     $  + drxnx_dprox(:)*dprodna(:)
C       endif
     ...
     $      + w/dz *(1.0d0-swex)   
     $     + (1.0d0-frex)*
     $  ksil(iz)*poro(iz)*hr(iz)*36.934d0*1d-6
     $   *rxnx(iz)
     $                )
C      $      * msx(iz)
...
C      $      *1d-10
     ...
     $     + (1.0d0-frex)*
     $      ksil(iz)*poro(iz)*hr(iz)
     $          *36.934d0*1d-6*msilx(iz)
     $   *rxnx(iz)
     $     + frex*
     $      ksil(iz)*poro(iz)*hr(iz)
     $          *36.934d0*1d-6*msil(iz)
     $   *rxn(iz)
     $                       )
...
C      $    + (1.0d0-frex)*
C      $      ksil(iz)*poro(iz)*hr(iz)
C      $          *36.934d0*1d-6*msilx(iz)
...
C      $  + (1.0d0-swex)*(v(iz)-v(iz-1))/dz
     $  + (1.0d0-frex)*(
     $  -(1.0d0)*ksil(iz)/sat(iz)*hr(iz)*36.934d0*1d-6*msilx(iz)
     $   *drxnx_dnax(iz)
     $           )
     $   *1d-3
     $                       )
...
C      $  + swex*(v(iz)-v(iz-1))*c(iz)/dz
     $  - (1.0d0-frex)*
     $   ksil(iz)/sat(iz)*hr(iz)*36.934d0*1d-6*msilx(iz)
     $   *rxnx(iz)
     $           *1d-3
     $  - frex*
     $   ksil(iz)/sat(iz)*hr(iz)*36.934d0*1d-6*msil(iz)
     $   *rxn(iz)
     $           *1d-3
     $                       )
     $    *merge(0.0d0,1.0d0,nax(iz)<nath)   ! commented out (is this necessary?)
...
C           amx3(row,row + 1) = (     
C      $   - (1.0d0-frex)*
C      $   ksil(iz)/sat(iz)*hr(iz)*36.934d0*1d-6*msilx(iz)
C      $   *(-4d0*nax(iz)**3d0*(-1d0)/(prox(iz)**2d0)/keqsil)
C      $   *merge(0d0,1d0
C      $  ,1d0-4d0*nax(iz)**3d0/prox(iz)/keqsil < 0d0)
C      $           *1d-3 
...
C         if (iz /= nz) then
          ...
          amx3(row,row  - 1) = (     
     $   - (1.0d0-frex)*
     $   ksil(iz)/sat(iz)*hr(iz)*36.934d0*1d-6*1d0
     $   *rxnx(iz)
     $           *1d-3 
     $                            )
     $        *msilx(iz)
...
C         dt = dt/1.05d0
     ...
     $                          )
      ...
      naeq = 1d0/(k2*k1*kco2*pco2i/prox/prox/kcceq)
      silsat = nax/naeq
...
C        print*,'z','na','msil','ph','sat'
C        do iz=1,nz 
C          print*,z(iz),nax(iz),msilx(iz),prox(iz),rxnx(iz)
C        enddo 
...
C          go to 100
      ...
      dporodta = 0d0
#ifdef poroevol    
      poro = poroi + (msili-msilx)*(36.934d0)*1d-6 
...
! ######################## end of pH iteration #######################
      ...
      !  endif of ifndef pyweath
       ...
       print *
       print *,'-=-=-=-=-=-= pH -=-=-=-=-=-=-='
       print *, 'ph:', (-log10(prox(iz)),iz=1,nz, spc)
       print *
#endif 
...
C       ca = 0d0
C #ifdef carbonate
C       caeq = 0.5d0*(kcceq/co3(nz)+ caeq)
C       caeq = min(caeq,1d-2)
C       ca = min(caeq,max(0d0,caeq*(z-zca)/delca))
C #endif 
...
C       if (any(it==reclis)) then
     ...
     $  *merge(0.0d0,1.0d0,po2x(iz)<po2th),
     $ merge(0.0d0,
     $      + koxs2(iz)*poro(iz)*hr(iz)*23.94d0*1d-6*msx(iz)
...
!       end if 
  ...
  999 continue
      ...
      subroutine calcpH(nz,nax,cx,c2x,so4x,k1,kco2,pco2i,k2,prox)
      implicit none 
      integer, intent(in)::nz 
      double precision,intent(in)::nax(nz),cx(nz),c2x(nz),so4x(nz)
      double precision,intent(in)::k1,kco2,pco2i,k2
      double precision,intent(inout)::prox(nz)
      
      double precision error_pro, tol_pro
      double precision fa(nz),fb(nz),fc(nz),fd(nz),fpro(nz),dfpro(nz)
      double precision dprox(nz)
      
      error_pro = 1d2
      tol_pro = 1d-12
      do while (error_pro>tol_pro)
      fa(:) = 1d0
      fb(:) = 2d0*nax(:)+2d0*cx(:)+3d0*c2x(:)-2d0*so4x(:)
      fc(:) = -k1*kco2*pco2i
      fd(:) = -2d0*k2*k1*kco2*pco2i
      fpro(:) = fa(:)*prox(:)**3d0 + fb(:)*prox(:)**2d0  
     $    + fc(:)*prox(:) + fd(:)  
      dfpro(:) = 3d0*fa(:)*prox(:)**2d0 + 2d0*fb(:)*prox(:)  
     $    + fc(:)
      dprox(:) = - fpro(:)/dfpro(:)
      error_pro = maxval(abs(dprox(:)/prox(:)))
      prox(:) = prox(:)+dprox(:)
      enddo 
