...
def ex_11_init():
    outdir_src = '/storage/coda1/p-creinhard3/0/ykanzaki3/scepter_output/Phreeqc_ex11_init/'
    ...
    for no3_pw_mM in [
        ...
        no3_pw = no3_pw_mM*1e-3
        
        rho = 258.162/99.52 * 1000 # kg/m3
        cec = 0.0011 * (0.5 * 1.0 * 1000 )/ ( (1.-0.5) * rho * 0.01 )
        
        sld_list = ['inrt']
        aq_list = ['na','k','ca','mg','no3','cl']
        # aq_list = ['na','k','ca','mg','fbc','cl']
        # gas_list = ['pco2']
        gas_list = []
        pr_list = [('inrt',1.0)]
        ...
        rstrt = 'self'
        runname = 'act_alpha{:.1f}_no3mM_q0_sense'.format(alpha).replace('.','p')
        runname = 'noact_alpha{:.1f}_no3mM_q0_sense'.format(alpha).replace('.','p')
        # runname = 'ex11_init_act_alpha{:.1f}_fbcmM_sense'.format(alpha).replace('.','p')
        ttot = 1
        rain_list = [
            ('na',na_pw), 
            ('k',k_pw), 
        ...
        # rstrt = 'ex11_init_act_nofh_nodisp'
        # runname = 'ex11_replace_act_nofh_nodisp'
        # ttot = 17.5
        ...
        # (1) get porewater pH 
        phint_field = get_int_prof.get_ph_int_site(outdir_src,runname,dep_sample)
        
        # (2) get acidity in %
        acint = get_int_prof.get_ac_int_site_v2(outdir_src,runname,dep_sample)
        
        res_list.append( [no3_pw_mM, phint_field, acint/100., 1. - acint/100.] )
    
    np.savetxt(runname+'.res',np.array(res_list))
