function test
    ...
    maq = ones(1,1)*1d-20;  % conc.list of aq  [mol/L]
    ...
    % added for PSD tracking 
    nps = 50; % number of particle size bins
    mpsd = ones(1,nps,1)*1d3; 
    % ^ psrticle numbers per m3 bulk soil per log(m) bin; 
    %        1st index --> number of solid sp., 2nd --> # of particle size bins; 3rd --> # depth layer (should be fixed at 1)  
    %        initial values do not matter as they are calculated when called first time and make sure volume conservation
    
    mgas = ones(1,1)*1d-2;
    pro = ones(1,1)*1d-5;     % H+ conc. [mol/L]
    so4f = ones(1,1)*1d-20;   % free SO4= [mol/L]
    tcin = 12;                % temperature [oC]
    chraq = string({'ca'});   % string list of aq phase
    chrsld = string({'cc'});  % string list of sld phase
    % chrgas = strings(0,1);
...
function [maq,mgas,msld,pro,so4f,poro,omega,mpsd,ps,agasx] = cell_out_psd( ...
    ...
    ,tcin ...% input 
    ,first_call ...% input 
    ,maq,mgas,msld,pro,so4f,mpsd ...% input 
    )

    %-----------------------------
    
    %%% previous input but now parameters not important for calculation in a cell
    nz = 1;
    rainpowder=0;
    zsupp=0;
    satup0=1;
    ...
    nsp_aq = size(chraq,1);
    nsp_sld = size(chrsld,1);
    nsp_gas = size(chrgas,1);
    nrxn_ext = size(chrrxn_ext,1);
    
    chrsld_kinspc_in ='';
    nsld_kinspc_in = size(chrsld_kinspc_in,1);
    kin_sld_spc_in = nan; 
    ...
    mv = zeros(nsp_sld,1,'double');
    mwt = zeros(nsp_sld,1,'double');
    for isps = 1: nsp_sld 
    ...
    maqi_all = ones(nsp_aq_all,1)*def_rain;
    
    % [msldi_all] = get_parentrock(nsp_sld_all,chrsld_all,def_pr);
    msldi_all = ones(nsp_sld_all,1)*def_pr;

    % bulk soil concentration     
    mblki = 0d0;
    incld_blk = false;
    
    %%%%%%%% The following is the modification of initial phase concentration and setting up bulk 'clay' phase 
    %%%%%%%% to satisfy volume conservation 
    %%%%%%%% Not to be considered for now for a box modelling
    
    % if ( sum(msldi_all) > 1d0)  % adding the case where input wt% exceeds 100% 
        % fprintf ( 'parent rock comp. exceeds 100%% so rescale\n')
        % msldi_all = msldi_all/sum(msldi_all);  % now the units are g/g
    % elseif ( sum(msldi_all) < 1d0 )  % when input is less than 100wt% add bulk soil 
    ...
    mgasi_all = zeros(nsp_gas_all,1,'double');
    mgasi_all(find(chrgas_all=='po2')) = 0.21d0;
    mgasi_all(find(chrgas_all=='pco2')) = 10d0^(-3.5d0);
    % mgasi_all(find(chrgas_all=='pnh3')) = 1d-9; % these may be too high 
    % mgasi_all(find(chrgas_all=='pn2o')) = 270d-9; % these may be too high 
    mgasi_all(find(chrgas_all=='pnh3')) = 1d-100; 
    mgasi_all(find(chrgas_all=='pn2o')) = 1d-100;
    ...
    rfrc_sld_all = zeros(nsp_sld_all,1,'double'); % just use default zero
    rfrc_sld_all(:) = def_dust;

    
    def_OM_frc = 0d0;
    % [rfrc_sld_plant_all] = get_OM_rain(nsp_sld_all,chrsld_all,def_OM_frc);
    % in default 
    rfrc_sld_plant_all = zeros(nsp_sld_all,1,'double');
    rfrc_sld_plant_all(:) = def_OM_frc;
    ...
    % above switching variables are now fixed 
    iwtype =0;
    imixtype =0;
    poroiter_in=false;
    display=false;
    display_lim_in=false;
    read_data=false;
    incld_rough=true;
    al_inhibit=false;
    timestep_fixed=false;
    ...
    [up,dwn,cnr,adf] = calcupwindscheme(w,nz);  %%%%%%%%%%%%%%%%%%%%%% turned off for explicit calculation (because nz = 1 and w becomes scalar 0)
    ...
    if (do_psd)  
        ...
        if (do_psd_full)  % do psd for every mienral
            ...
            for isps = 1: nsp_sld
                ...
                volsld = msld(isps,1)*mv(isps)*1d-6;
                [psd_pr] = calc_psd_pr( ...
                    nps ...% input
    ...
    dt = ttot; % for basalt exp?

    %%%%%%%% now using input so ignore initilization here
    ...
    %%%%%%%% as above imaginary bulk soil phase is ignore for now
    ...
    %%%%%%%% using input now so ignore initilization 
...
% 100 continue
        ...
        % mblkx = mblk; %%%%%%%%%%%%%%%%%% Tuned off for explcit calculation 
        ...
        if (do_psd) 
            ...
            if (do_psd_full) 
                ...
                for isps = 1: nsp_sld
                    ...
                    DV(:) = ( msld(isps,:) - msldx(isps,:) )*mv(isps)*1d-6;   % for explicit calculation 
            ...
            mpsd(:,:,:) = 0d0;
                        ...
                        break %%%%%%%%%%%%%%%%%%%%%%%%%% for explicit calculation %%%%%%%%%%%%%%%%%%%%%%%
    ...
    while ((~isnan(error_dum))&&(error_dum > tol*fact_tol))
        ...
        if (~sld_enforce)  
            ...
            for iz = 1: nz  %================================
                ...
                for isps = 1: nsp_sld
                    ...
                    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% explicit solution
                    msldx_exp(isps,iz) =  ...
                        + (sporo_tmp*m_tmp/merge(1d0,dt,dt_norm) - ymx3(row))*merge(1d0,dt,dt_norm)/sporo_tmp;
                    % disp(sporo_tmp*m_tmp/merge(1d0,dt,dt_norm));
        ...
        for iz = 1: nz
            ...
            for ispa = 1: nsp_aq
                ...
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% explicit solution
                maqx_exp(ispa,iz) = ...
                    + (poro(iz)*sat(iz)*1d3*caq_tmp/merge(1d0,dt,dt_norm) - ymx3(row))*merge(1d0,dt,dt_norm)/(poro(iz)*sat(iz)*1d3);
        ...
        for iz = 1: nz
            ...
            for ispg = 1: nsp_gas
                ...
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% explicit solution
                mgasx_exp(ispg,iz) = ...
                    + (agasx(ispg,iz)*mgasx(ispg,iz)/merge(1d0,dt,dt_norm) - ymx3(row))*merge(1d0,dt,dt_norm)/agasx(ispg,iz);
            end 
...
function [ ...
    ...
    psdx_exp=zeros(nps,nz,'double');  % for explicit calculation 
    ...
    % while ( any (emx3 > fact_tol )  ) 
        ...
        for ips = 1: nps
            ...
            for iz = 1: nz
                ...
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% explicit solution
                psdx_exp(ips,iz) =  ...
                    + (sporo_tmp*m_tmp*merge(1d0,dt,dt_norm) - ymx3(row))/merge(1d0,dt,dt_norm)/sporo_tmp  /( vol * dps(ips));
